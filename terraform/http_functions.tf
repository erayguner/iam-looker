...
resource "google_cloudfunctions2_function" "http_create_folder" {
  ...
  service_config { max_instance_count = 3 available_memory = "256M" timeout_seconds = 60 ingress_settings = "ALLOW_INTERNAL_ONLY" environment_variables = { LOOKERSDK_BASE_URL = var.looker_base_url LOOKERSDK_CLIENT_ID = var.looker_client_id LOOKERSDK_CLIENT_SECRET = var.looker_client_secret } }
}
...
resource "google_cloudfunctions2_function" "http_create_dashboard" {
  ...
  service_config { max_instance_count = 5 available_memory = "512M" timeout_seconds = 120 ingress_settings = "ALLOW_INTERNAL_ONLY" environment_variables = { LOOKERSDK_BASE_URL = var.looker_base_url LOOKERSDK_CLIENT_ID = var.looker_client_id LOOKERSDK_CLIENT_SECRET = var.looker_client_secret } }
}
